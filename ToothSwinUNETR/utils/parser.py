...
parser.add_argument("--patch_mode",
                    type=str,
                    default="local",
                    choices=["local", "global"],
                    help="Global - crop on random patches from the whole scan, or local - use predefined cropped rois with single teeth.")
...
parser.add_argument("--labels_folder",
                    type=str,
                    default="labels_american",
                    help="Path to the labels.")
parser.add_argument("--model_name",
                    type=str,
                    default="AttUnet",
                    choices=["UNet", "VNet", "AttUnet", "UNETR", "SwinUNETR"],
                    help="Type of the model.")
parser.add_argument("--scheduler_name",
                    type=str,
                    default="warmup",
                    choices=["annealing", "warmup", "warmup_restarts"],
...
parser.add_argument("--loss_name",
                    type=str,
                    default="DiceLoss",
                    choices=["DiceCELoss", "DiceLoss", "WasserDiceLoss"],
...
parser.add_argument("--include_background",
                    type=bool,
                    default=False,
                    help="If include background in loss citerion and metric.")
...
parser.add_argument("--multitask",
                    type=bool,
                    default=False,
                    help="Add binary task to the segmentation.")
parser.add_argument("--boundary_loss",
                    type=bool,
                    default=False,
                    help="Use soft labels for gaussian boundary.")
parser.add_argument("--boundary_cls_loss",
                    type=bool,
                    default=True,
                    help="Boundary classification loss.")
parser.add_argument("--loss_weights",
                    type=dict,
                    default={'bin_w':1.0, 'ce_w':1.0, "dice_w": 1.0, 'gwdl_w': 1.0, 'bnd_mse_w': 0.0, 'bnd_dice_w': 0.2, 'bnd_ce_w': 0.2, 'tooth_id_w': 0.1},
                    help="Multitask loss weights.")
...
parser.add_argument("--use_scaler",
                    type=bool,
                    default=False,
                    help="Use amp scaler FP16 to speed up training.")
...
parser.add_argument("--checkpoint_dir",
                    type=str,
                    default='ablation_study/local_ablation/loose_roi_local',
